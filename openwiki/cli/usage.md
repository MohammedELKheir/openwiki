...
- a **provider** (`OPENWIKI_PROVIDER`) — openai, openai-chatgpt, openrouter, baseten, fireworks, nvidia, openai-compatible, anthropic, or zai,
- the **provider API key** (e.g. `OPENROUTER_API_KEY`, `OPENAI_API_KEY`, `OPENAI_COMPATIBLE_API_KEY`, `ANTHROPIC_API_KEY`, `ZAI_API_KEY`, `BASETEN_API_KEY`, `FIREWORKS_API_KEY`, `NVIDIA_API_KEY`),
...
| nvidia            | `NVIDIA_API_KEY`              | `https://integrate.api.nvidia.com/v1`   | Nemotron, DeepSeek V4 Pro, GPT-OSS, Kimi K2.6                         |
...
| zai               | `ZAI_API_KEY`                 | `https://api.z.ai/api/anthropic`        | GLM 5.2, GLM 5.1                                                      |

The default provider is `openai`, and the default model is `gpt-5.6-terra`. `resolveConfiguredProvider()` picks the provider from `OPENWIKI_PROVIDER`, then falls back to the first configured provider API key in this order: Z.AI, OpenAI, OpenAI-compatible, OpenRouter, Anthropic, Baseten, Fireworks, NVIDIA NIM, and finally `DEFAULT_PROVIDER`.
